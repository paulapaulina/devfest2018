    ...
    return firebase.firestore().collection('tickets')
      ...
      .then((snaps) => {
        const list = snaps.docs
          .map((snap) => Object.assign({}, snap.data(), {
            id: snap.id
          }));
...
const partnersActions = {
  ...
  fetchPartners: () => (dispatch) => {
    ...
    firebase.firestore()
      ...
      .then((groups) => groups.map(([group, id, items]) => Object.assign({}, group, {
        id,
        items
      })))
      .then((list) => {
  ...
  fetchSchedule: () => (dispatch, getState) => {
    ...
    return Promise.all([speakersPromise, schedulePromise])
      .then(([speakers, schedule]) => {
        ...
        scheduleWorker.addEventListener('message', ({
          data
        }) => {
          dispatch({
...
const teamActions = {
  fetchTeam: () => (dispatch) => {
    ...
    firebase.firestore()
      ...
      .then((teams) => teams.map(([team, id, members]) => Object.assign({}, team, {
        id,
        members
      })))
      .then((list) => {
